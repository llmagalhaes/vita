...
Phase 2 — contract v0.3.0 built & tested LOCALLY; app↔backend core loop proven E2E against real Postgres. Backend BE-005–016 done (84 tests, ADR-0012 layout); app walkable in store Expo Go SDK 56; AWS parked (~$6/mo idle). **Active backlog: "Vita 100% local" — `docs/backlog-local-100.md` rev 3, EXECUTION READY.** All 32 tickets in Asana (BE-017–028, APP-017–035/037, OPS-020; BE-017/BE-023/APP-017 in To do). Slices 1–8: water → workout → plan/program (history ≤5, fully editable) → habits/notifications (check-ins persisted) → photo → trends → settings/vacation/export → debt + LocalStack adapters. No GitHub CI/CD; LocalStack instead of AWS; Terraform kept ready. Release pipeline AFTER local-100 (CEO R11): hygiene sweep (BE-028/APP-037, parked) → AWS deploy → Android vs AWS → iOS on iPhone vs AWS → Play Store → App Store. Next action: start slice 1 (APP-017) + BE-017/BE-023 in parallel. Sessions run on Opus. Details in `Next_session.md` (root).
