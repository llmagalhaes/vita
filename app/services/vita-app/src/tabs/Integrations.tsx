/**
 * Integrations (APP-029 · cleaned up APP-072 · moved to a tab in APP-084). Health
 * Connect is the only real health-source integration Vita has, and it is Android-
 * only — so this screen shows ONLY Health Connect, and only on Android. Now the
 * 6th nav position: rendered by TabsPager; the /integrations route is a placeholder.
 */
import { useMemo } from "react";
import { Platform, ScrollView, View } from "react-native";
import { useTranslation } from "react-i18next";
import { useRouter } from "expo-router";
import { BackButton, Card, Text, Toggle, colors, fonts } from "../ui";
...
export default function Integrations() {
  ...
  return (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingHorizontal: 22, paddingTop: 60, paddingBottom: 150, gap: 12 }}>
      <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
        <BackButton onPress={() => (router.canGoBack() ? router.back() : router.replace("/home"))} label={t("account.back")} />
