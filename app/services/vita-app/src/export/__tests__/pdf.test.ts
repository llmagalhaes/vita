...
const t = (k: string) => (k === "common.ml" ? "ml" : k === "common.l" ? "L" : k === "common.oz" ? "oz" : k);
...
test("builds HTML from local SQLite entries with the selected sections", () => {
  seed();
  const html = buildExportHtml(allEntries(), { audienceLabel: "My doctor", sections: ["meals", "water", "energy"], units: "metric", t });
...
test("estimates are labeled in the PDF", () => {
  seed();
  const html = buildExportHtml(allEntries(), { audienceLabel: "Just for me", sections: ["meals"], units: "metric", t });
...
test("only the chosen sections appear (workouts excluded here)", () => {
  seed();
  const html = buildExportHtml(allEntries(), { audienceLabel: "My nutritionist", sections: ["meals", "macros", "water"], units: "metric", t });
...
test("user free-text is HTML-escaped (trust boundary)", () => {
  ...
  const html = buildExportHtml(allEntries(), { audienceLabel: "Me", sections: ["meals"], units: "metric", t });
