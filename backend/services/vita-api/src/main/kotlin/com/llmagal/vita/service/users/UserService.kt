...
import com.llmagal.vita.repository.users.UserRepository
...
class UserService(
    ...
    fun get(userId: UUID): UserResponse {
        ...
        return UserResponse(
            id = row.id,
            name = row.nameEnc?.let { String(crypto.decryptForUser(userId, AadContext.USER_NAME, it)) } ?: "",
    ...
    fun update(
        ...
        req.name?.let { name ->
            ...
            repo.updateName(userId, crypto.encryptForUser(userId, AadContext.USER_NAME, name.toByteArray()))
        }
        req.units?.let { units ->
