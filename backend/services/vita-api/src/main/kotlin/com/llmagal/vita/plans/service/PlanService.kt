package com.llmagal.vita.plans.service

import com.llmagal.vita.crypto.service.CryptoService
import com.llmagal.vita.plans.controller.PlanVersion
import com.llmagal.vita.plans.repository.PlanRepository
import com.llmagal.vita.plans.repository.PlanTable
...
class PlanService(
    ...
    fun importVersion(
        ...
        val stored = repo.insert(table, userId, encrypt(userId, doc))
        repo.trim(table, userId, historyMax)
        return decode(userId, stored.docEnc)
    }
    ...
    fun current(
        table: PlanTable,
        userId: UUID,
    ): JsonNode? = repo.current(table, userId)?.let { decode(userId, it.docEnc) }
    ...
    fun edit(
        ...
        userId: UUID,
        doc: Any,
    ): JsonNode? = repo.updateCurrent(table, userId, encrypt(userId, doc))?.let { decode(userId, it.docEnc) }
    ...
    fun history(
        ...
        repo.history(table, userId, historyMax).map {
            PlanVersion(it.id, it.createdAt, decode(userId, it.docEnc))
        }

    private fun encrypt(
        userId: UUID,
        doc: Any,
    ): ByteArray = crypto.encryptForUser(userId, mapper.writeValueAsBytes(doc))

    private fun decode(
        userId: UUID,
        blob: ByteArray,
    ): JsonNode = mapper.readTree(crypto.decryptForUser(userId, blob))
}
