...
import com.llmagal.vita.users.repository.VacationRepository
...
class VacationService(
    ...
    fun get(userId: UUID): JsonNode =
        repo.find(userId)?.let { mapper.readTree(crypto.decryptForUser(userId, it)) }
    ...
    fun replace(
        ...
        repo.upsert(userId, crypto.encryptForUser(userId, mapper.writeValueAsBytes(ranges)))
        return ranges
    }
}
